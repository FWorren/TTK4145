...
func Elevator_wait() {
	for {
		...
		if OrderLogic_get_number_of_orders() > 0 {State = RUN}
		if Elev_get_stop_signal() {State = STOPS}
		if State != WAIT || State == STOPS{break}
	}
}
...
func Elevator_run() {
	...
	for {
		...
		if Elev_get_stop_signal() || Elev_get_obstruction_signal() {
			State = STOPS
			Elev_set_speed(0)
			break
		}
...
func Elevator_door() {
	...
	for {
		if !Elev_get_obstruction_signal() {break}
	}
